 ...
 * -------------------------
 * 
 * Processing.jx - Arcux Sistemas S.A.
 * MIT Licensed
 * http://www.arcux.com/processingjx/
 *
 */
...
function buildProcessing( curElement ){
  ...
  // processing.jx - Stroke constants 
  p.ROUND="round";
  p.SQUARE="square";
  p.PROJECT="butt";
  p.BEVEL="bevel";
  ...
  // processing.jx - Float type constants
  p.MAX_FLOAT = 99999999999999;
  p.MIN_FLOAT = -99999999999999;
  var Float = {};
  Float.MAX_VALUE = 99999999999999;
  Float.MIN_VALUE = -99999999999999;
  Float.isNaN = function(value) {
    return isNaN(value);
  };
  ...
  CanvasTextFunctions.enable(curContext);
  var doFill = true;
  var doStroke = true;
  ...
  var curTint = -1;
  var curTextSize = 12;
  var curTextHorzAlign = p.LEFT;
  var curTextVertAlign = p.BASELINE;
  var curTextLeading = 10;
  ...
  // arcux processing fork 
  p.split = function split(args,delimiter) {
    return args.split(delimiter);
  }
  
  // arcux processing fork 
  p.trim = function trim(s) {
    return s.replace(/^\s+|\s+$/g, '');
  }
  
  p.subset = function subset(array, iIndexA, iIndexB) {
  ...
  // processing.jx - loadString function fixed
  p.loadStrings = function loadStrings(loadUrl) {
    if ('undefined' == typeof(loadUrl)) return false;
    var px;
    if (document.all){
      // For IE, create an ActiveX Object instance 
      px = new ActiveXObject("Microsoft.XMLHTTP");
    } else {
   // For mozilla, create an instance of XMLHttpRequest.
     px = new XMLHttpRequest();
    }    
    px.open('get',loadUrl,false);
    px.send('');
    return px.responseText.split("\n");
  };
  ...
  p.loadFont = function loadFont( name, size  ) {
    curTextFont.name = name;
    p.textSize( size );
  };

  // processing.jx fix 
  p.createFont = function createFont(name,size) {
  ...
  // Processing.jx - text functions fixed
  
  p.textWidth = function textWidth( str ) {
    return curContext.measureText(curTextFont.name, curTextSize, str);
  };
  
  p.textAscent = function textAscent()  {
    return curContext.fontAscent(curTextFont.name, curTextSize);
  };
  
  p.textDescent = function textDescent()  {
    return curContext.fontDescent(curTextFont.name, curTextSize);
  };
  
  p.textLeading = function textLeading( size) { 
    if ( size ) {
      curTextLeading = size;
    }
  };
  
  p.textAlign = function textAlign( horiz, vert ) {
    if ( horiz )
      curTextHorzAlign = horiz;
    if ( vert )
      curTextVertAlign = vert;
    else
      curTextVertAlign = p.BASELINE;
  };
  
  // Processing.jx - text function multibrowser compatibility
  p.text = function text( str, x, y ) {
    str = str+"";
    var temp = curContext.strokeStyle;
    var offset=0;
    curContext.strokeStyle = curContext.fillStyle;
    if (curTextVertAlign==p.TOP)
      offset =  p.textAscent();
    else if (curTextVertAlign==p.BOTTOM)
      offset = - p.textDescent();
    else if (curTextVertAlign==p.CENTER)
      offset = p.textAscent()/2;
    var lines = str.split("\n");
    for (var i=0; i<lines.length;i++) {
    if (curTextHorzAlign==p.LEFT)
      curContext.drawText( curTextFont.name, curTextSize, x, y+offset+curTextLeading*i,lines[i]);
    else if (curTextHorzAlign==p.RIGHT)
      curContext.drawTextRight( curTextFont.name, curTextSize, x, y+offset+curTextLeading*i,lines[i]);
    else if (curTextHorzAlign==p.CENTER)
      curContext.drawTextCenter( curTextFont.name, curTextSize, x, y+offset+curTextLeading*i,lines[i]);
    }
    curContext.strokeStyle = temp;
  };
  ...
  p.parseInteger = function parseInteger( string ) {
    return parseInt(string);
  }
  
  // Processing.jx - float function fixed
  p.float = function float( aNumber ) {
    if (typeof aNumber == "number")
      return Math.pow(aNumber,1)
    else if (typeof aNumber == "string")
      return window.parseFloat( aNumber );
    else if (typeof aNumber == "object") {
      var numArray = new Array();
      for (i=0; i<aNumber.length;i++) {
        numArray[i]=p.float(aNumber[i]);
      }
      return numArray;
    }
  }
 
  // Processing.jx - parseFloat function fixed
  p.parseFloat = function parseFloat(value) {
  ...
  // Processing.jx - stroke functions fixed
  p.strokeCap = function strokeCap(cap) {
    curContext.lineCap = cap;
  }
   
  p.strokeJoin = function strokeJoin(join) {
    curContext.lineJoin = join;
  }
  
  p.point = function point( x, y ) {
    var oldFill = curContext.fillStyle;
    var width = curContext.lineWidth;
    curContext.fillStyle = curContext.strokeStyle;
    curContext.fillRect( Math.round(x-width/2), Math.round(y-width/2), width, width);
    curContext.fillStyle = oldFill;
  };
  ...
  // processing.jx - Line function fixed
  ...
  p.ellipse = function ellipse( x, y, width, height ) {
    ...
    if ( width == height )
      ...
      // Ellipse code by Gavin Doughtie and Jon Stewart’s
      var KAPPA = 4 * ((Math.sqrt(2) -1) / 3);
      var rx = width/2;
      var ry = height/2;

      curContext.beginPath();
      curContext.moveTo(x, y - ry);
      curContext.bezierCurveTo(x + (KAPPA * rx), y - ry,  x + rx, y - (KAPPA * ry), x + rx, y);
      curContext.bezierCurveTo(x + rx, y + (KAPPA * ry), x + (KAPPA * rx), y + ry, x, y + ry);
      curContext.bezierCurveTo(x - (KAPPA * rx), y + ry, x - rx, y + (KAPPA * ry), x - rx, y);
      curContext.bezierCurveTo(x - rx, y - (KAPPA * ry), x - (KAPPA * rx), y - ry, x, y - ry);
    }
...
// processing.jx - String functions
String.prototype.startsWith=function startsWith(ch) {
    if (this.charAt(0)==ch) return true;
    else return false;
}

String.prototype.equals=function equals(s) {
    return (this==s);
}
